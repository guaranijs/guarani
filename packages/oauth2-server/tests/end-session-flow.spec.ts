...
import { EndSessionRequest } from '../src/lib/requests/end-session-request';
...
import { LogoutContextInteractionRequest } from '../src/lib/requests/interaction/logout-context.interaction-request';
import { LogoutDecisionAcceptInteractionRequest } from '../src/lib/requests/interaction/logout-decision-accept.interaction-request';
...
import { LogoutContextInteractionResponse } from '../src/lib/responses/interaction/logout-context.interaction-response';
import { LogoutDecisionInteractionResponse } from '../src/lib/responses/interaction/logout-decision.interaction-response';
...
describe('End Session Flow', () => {
  ...
  it('GET /oauth/authorize', async () => {
    ...
    expect(consentInteractionContextResponse.body).toMatchObject<Partial<ConsentContextInteractionResponse>>({
      skip: false,
      requested_scope: 'openid profile email phone address foo bar baz qux',
      subject: '16907c32-687b-493c-85ba-f41f2c9d4daa',
    ...
    const consentInteractionAcceptDecisionRequestData: ConsentDecisionAcceptInteractionRequest = {
      ...
      consent_challenge: consentChallenge,
      decision: 'accept',
      grant_scope: 'openid profile email phone address foo bar baz qux',
    };
  ...
  it('POST /oauth/end_session', async () => {
    // #region Retrieve the Logout Challenge and Logout Ticket
    const endSessionRequestData: EndSessionRequest = {
      id_token_hint: idToken,
      client_id: 'b1eeace9-2b0c-468e-a444-733befc3b35d',
      post_logout_redirect_uri: 'http://localhost:4000/oauth/logout_callback',
      state: '2f3ebe49f9ade4bc8b6d90f4da3480a8',
      logout_hint: 'john.doe@email.com',
    };

    const endSessionRequestSearchParameters = stringifyQs(endSessionRequestData);

    const endSessionResponse = await agent.get(`/oauth/end_session?${endSessionRequestSearchParameters}`);

    const logoutUrl = new URL(endSessionResponse.headers.location);
    const logoutChallenge = logoutUrl.searchParams.get('logout_challenge')!;

    expect(endSessionResponse.status).toEqual(303);
    expect(logoutChallenge).not.toBeEmpty();

    expect(agent.jar.getCookie('guarani:session', CookieAccessInfo.All)?.value).toEqual(expect.any(String));
    expect(agent.jar.getCookie('guarani:logout', CookieAccessInfo.All)?.value).toEqual(expect.any(String));
    // #endregion

    // #region Get the Logout Interaction Status
    const logoutContextInteractionRequestData: LogoutContextInteractionRequest = {
      interaction_type: 'logout',
      logout_challenge: logoutChallenge,
    };

    const logoutContextInteractionRequestSearchParameters = stringifyQs(logoutContextInteractionRequestData);

    const logoutContextInteractionResponse = await agent.get(
      `/oauth/interaction?${logoutContextInteractionRequestSearchParameters}`,
    );

    expect(logoutContextInteractionResponse.status).toEqual(200);

    expect(logoutContextInteractionResponse.body).toStrictEqual<LogoutContextInteractionResponse>({
      skip: false,
      request_url: logoutContextInteractionResponse.body.request_url,
      client: 'b1eeace9-2b0c-468e-a444-733befc3b35d',
      context: { logout_hint: 'john.doe@email.com' },
    });
    // #endregion

    // #region Execute the Logout
    const logoutDecisionAcceptInteractionRequestData: LogoutDecisionAcceptInteractionRequest = {
      interaction_type: 'logout',
      logout_challenge: logoutChallenge,
      decision: 'accept',
      session_id: agent.jar.getCookie('guarani:session', CookieAccessInfo.All)!.value.substring(4, 40),
      logout_type: 'local',
    };

    const logoutDecisionAcceptInteractionBody = stringifyQs(logoutDecisionAcceptInteractionRequestData);

    const logoutDecisionAcceptInteractionResponse = await agent
      .post('/oauth/interaction')
      .set('Content-Type', 'application/x-www-form-urlencoded')
      .set('Accept', 'application/json')
      .send(logoutDecisionAcceptInteractionBody);

    expect(logoutDecisionAcceptInteractionResponse.status).toEqual(200);

    expect(logoutDecisionAcceptInteractionResponse.body).toStrictEqual<LogoutDecisionInteractionResponse>({
      redirect_to: logoutContextInteractionResponse.body.request_url,
    });
    // #endregion

    // #region Call End Session Endpoint to clear the Logout Ticket on the cookies and get the Logout Callback URL
    const cookieClearEndSessionResponse = await agent.get(`/oauth/end_session?${endSessionRequestSearchParameters}`);

    expect(cookieClearEndSessionResponse.status).toEqual(303);

    expect(cookieClearEndSessionResponse.headers.location).toEqual(
      'http://localhost:4000/oauth/logout_callback?state=2f3ebe49f9ade4bc8b6d90f4da3480a8',
    );

    expect(agent.jar.getCookie('guarani:session', CookieAccessInfo.All)?.value).toEqual(expect.any(String));
    expect(agent.jar.getCookie('guarani:logout', CookieAccessInfo.All)?.value).toBeUndefined();
    // #endregion
  });
});

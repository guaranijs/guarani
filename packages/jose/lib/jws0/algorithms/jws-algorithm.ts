...
export abstract class JWSAlgorithm {
   ...
   */
  public constructor(protected readonly hash: Nullable<SupportedHash>, protected readonly algorithm: string) {}
   ...
   */
  public abstract verify(signature: string, message: Buffer, key?: JsonWebKey): Promise<void>;
  ...
  protected checkKey(key: JsonWebKey): void {
    ...
    if (key.alg && key.alg !== this.algorithm) {
      throw new InvalidJsonWebKeyException(`This key is intended to be used by the algorithm "${key.alg}".`);
    }

    if (key.kty !== this.kty) {
      throw new InvalidJsonWebKeyException(`This algorithm only accepts "${this.kty}" keys.`);
    }
  }
}

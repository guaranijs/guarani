 ...
 */
export function encodeLength(length: number | bigint): Buffer {
  if (typeof length !== 'number' && typeof length !== 'bigint') {
    throw new TypeError('Invalid parameter "length".');
  }

  const data = BigInt(length);

  if (data < 0x80n) {
    return integerToBuffer(data);
  }

  const asBuffer = integerToBuffer(data);

  if (asBuffer.length > 0x7f) {
 ...
 */
export function decodeLength(data: Buffer): bigint {
  if (!Buffer.isBuffer(data)) {
    throw new TypeError('Invalid parameter "data".');
  }

  let offset = 0;
  let length = BigInt(data[offset++]);

  // Long form
  if ((length & 0x80n) !== 0x00n) {
    const bytes = Number(length & 0x7fn);

    length = 0n;

    for (let i = 0; i < bytes; i++) {
      length = (length << 8n) | BigInt(data[offset + i]);
    }
  }

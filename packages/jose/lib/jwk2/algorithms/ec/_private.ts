 ...
 */
export function decodePrivateSec1(decoder: Decoder, options?: JsonWebKeyParams): EcKey {
  ...
  const curve = Object.values(ELLIPTIC_CURVES).find((curve) => Buffer.compare(curve.buffer, curveOid) === 0);
 ...
 * @returns Instance of an EcKey.
 */
export function decodePrivatePkcs8(decoder: Decoder, options?: JsonWebKeyParams): EcKey {
...
export function encodePrivateSec1(key: EcKey): Node<any> {
  ...
  return new SequenceNode([
    ...
    new ContextSpecific(0x00, 'constructed', new ObjectIdNode(curve.oid).encode()),
    new ContextSpecific(0x01, 'constructed', new BitStringNode(publicKey).encode()),
  ]);
}
...
export function encodePrivatePkcs8(key: EcKey): Node<any> {
  ...
  return new SequenceNode([
    ...
    new SequenceNode([new ObjectIdNode('1.2.840.10045.2.1'), new ObjectIdNode(curve.oid)]),

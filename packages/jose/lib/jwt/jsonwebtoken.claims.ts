...
import { ExpiredToken, InvalidJsonWebTokenClaim, TokenNotValidYet } from '../exceptions';
...
export class JsonWebTokenClaims implements JWTClaims {
   ...
   */
  public constructor(claims: JsonWebTokenClaims, options?: Dict<JWTClaimOptions>);
  ...
  public constructor(claims: JsonWebTokenClaims | JWTClaims, options?: Dict<JWTClaimOptions>) {
   ...
   */
  private validateClaimsOptions(claims: JWTClaims, options: Dict<JWTClaimOptions>): void {
    Object.entries(options).forEach(([claim, option]) => {
      ...
      if ('value' in option && !isDeepStrictEqual(value, option.value)) {
        throw new InvalidJsonWebTokenClaim(`Mismatching expected value for claim "${claim}".`);
      }

      if ('values' in option) {
        if (!Array.isArray(option.values)) {
          throw new InvalidJsonWebTokenClaim('Expected an array for the option "values".');
        }

        if (Array.isArray(value) && value.some((item) => !option.values!.includes(item))) {
          throw new InvalidJsonWebTokenClaim(
            'One or more received values are not present ' + `in the expected values of the claim "${claim}".`
          );
        }

        if (typeof value === 'string' && !option.values.includes(value)) {
          throw new InvalidJsonWebTokenClaim(
            `Mismatching expected value for claim "${claim}". ` + `Received "${value}".`
          );
        }
      }
    });

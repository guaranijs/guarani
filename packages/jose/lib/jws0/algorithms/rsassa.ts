...
class RSASSAAlgorithm extends JWSAlgorithm {
  ...
  public async sign(message: Buffer, key: RsaKey): Promise<string> {
    ...
    return b64Url.encode(sign(this.hash, message, { key: privateKey, padding: this.padding }));
  }
   ...
   */
  public async verify(signature: string, message: Buffer, key: RsaKey): Promise<void> {
    this.checkKey(key);

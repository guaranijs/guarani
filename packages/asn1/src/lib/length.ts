 ...
 * @param long Indicates if a length smaller than 128 has to be encoded using the Long Form.
 * @returns Buffer object representing the encoded length.
 */
export function encodeLength(length: number, long?: true): Buffer {
 ...
 *
 * If the MSB is 0, it is assumed to be a Short Form, where the byte represents the actual value of the length.
 *
 * If the MSB is 1, it is assumed to be a Long form, where the rest of the bits represent the number of bytes
 * used to represent the length, up to a maximum of 127 bytes, and the following bytes are then treated
 * entirely as one number representing the actual length.
 *
...
export function decodeLength(data: Buffer): number {
  ...
  if (length === 0xff) {
    throw new DeserializationException('Invalid length long form.');
  }

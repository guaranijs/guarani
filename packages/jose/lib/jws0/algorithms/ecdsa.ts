...
class ECDSAAlgorithm extends JWSAlgorithm {
   ...
   */
  public async verify(signature: string, message: Buffer, key: EcKey): Promise<void> {
    this.checkKey(key);

    const publicKey = createPublicKey(key.export('public', 'pem'));

    if (!verify(this.hash, message, publicKey, b64Url.decode(signature, Buffer))) {
  ...
  protected checkKey(key: EcKey): void {
    ...
    if (key.crv !== this.curve) {
      throw new InvalidKey(`This algorithm only accepts the curve "${this.curve}".`);
    }
  }
}

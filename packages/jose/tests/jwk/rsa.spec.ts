...
describe('RSA Public Key', () => {
  ...
  it('should reject keys with a wrong parameter "n".', () => {
    ...
    // @ts-expect-error
    expect(() => new RsaKey({ n: 123, ...params })).toThrow('Invalid parameter "n".');

    expect(() => new RsaKey({ n: 'modulus', ...params })).toThrow('The modulus MUST have AT LEAST 2048 bits.');
  });
  ...
  it('should reject keys with a wrong parameter "e".', () => {
    ...
    // @ts-expect-error
    expect(() => new RsaKey({ e: 123, ...params })).toThrow('Invalid parameter "e".');
  });
...
describe('RSA Private Key', () => {
  ...
  it('should reject keys with a wrong parameter "d".', () => {
    ...
    // @ts-expect-error
    expect(() => new RsaKey({ d: 123, ...params })).toThrow('Invalid parameter "d".');
  });
  ...
  it('should reject keys with a wrong parameter "p".', () => {
    ...
    // @ts-expect-error
    expect(() => new RsaKey({ p: 123, ...params })).toThrow('Invalid parameter "p".');
  });
  ...
  it('should reject keys with a wrong parameter "q".', () => {
    ...
    // @ts-expect-error
    expect(() => new RsaKey({ q: 123, ...params })).toThrow('Invalid parameter "q".');
  });
  ...
  it('should reject keys with a wrong parameter "dp".', () => {
    ...
    // @ts-expect-error
    expect(() => new RsaKey({ dp: 123, ...params })).toThrow('Invalid parameter "dp".');
  });
  ...
  it('should reject keys with a wrong parameter "dq".', () => {
    ...
    // @ts-expect-error
    expect(() => new RsaKey({ dq: 123, ...params })).toThrow('Invalid parameter "dq".');
  });
  ...
  it('should reject keys with a wrong parameter "qi".', () => {
    ...
    // @ts-expect-error
    expect(() => new RsaKey({ qi: 123, ...params })).toThrow('Invalid parameter "qi".');
  });
...
describe('RsaKey generate()', () => {
  it('should reject an invalid modulus length.', async () => {
    await expect(RsaKey.generate(2048.16)).rejects.toThrow('Invalid modulus length.');
  });

  it('should reject a modulus length smaller than 2048 bits.', async () => {
    await expect(RsaKey.generate(2047)).rejects.toThrow('The modulus MUST be AT LEAST 2048 bits long.');
  });
...
describe('RsaKey export()', () => {
  ...
  it('should reject an invalid key.', () => {
    // @ts-expect-error
    expect(() => key.export('invalid', 'pem', 'pkcs1')).toThrow('Invalid parameter "key".');
  });

  it('should reject an invalid public key type.', () => {
    // @ts-expect-error
    expect(() => key.export('public', 'pem', 'pkcs8')).toThrow('Invalid parameter "type".');
  });

  it('should reject an invalid private key type.', () => {
    // @ts-expect-error
    expect(() => key.export('private', 'pem', 'x509')).toThrow('Invalid parameter "type".');
  });

  it('should export a DER PKCS#1 RSA Public Key.', () => {
    expect(key.export('public', 'der', 'pkcs1')).toEqual(publicKey.export({ format: 'der', type: 'pkcs1' }));
  });

  it('should export a PEM PKCS#1 RSA Public Key.', () => {
    expect(key.export('public', 'pem', 'pkcs1')).toEqual(publicKey.export({ format: 'pem', type: 'pkcs1' }));
  });

  it('should export a DER X.509 SubjectPublicKeyInfo RSA Public Key.', () => {
    expect(key.export('public', 'der', 'x509')).toEqual(publicKey.export({ format: 'der', type: 'spki' }));
  });

  it('should export a PEM X.509 SubjectPublicKeyInfo RSA Public Key.', () => {
    expect(key.export('public', 'pem', 'x509')).toEqual(publicKey.export({ format: 'pem', type: 'spki' }));
  });

  it('should export a DER PKCS#1 RSA Private Key.', () => {
    expect(key.export('private', 'der', 'pkcs1')).toEqual(privateKey.export({ format: 'der', type: 'pkcs1' }));
  });

  it('should export a PEM PKCS#1 RSA Private Key.', () => {
    expect(key.export('private', 'pem', 'pkcs1')).toEqual(privateKey.export({ format: 'pem', type: 'pkcs1' }));
  });

  it('should export a DER PKCS#8 RSA Private Key.', () => {
    expect(key.export('private', 'der', 'pkcs8')).toEqual(privateKey.export({ format: 'der', type: 'pkcs8' }));
  });

  it('should export a PEM PKCS#8 RSA Private Key.', () => {
    expect(key.export('private', 'pem', 'pkcs8')).toEqual(privateKey.export({ format: 'pem', type: 'pkcs8' }));
  });
});

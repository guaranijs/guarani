...
describe('JSON Web Key', () => {
  ...
  describe('thumbprint', () => {
    ...
    const key: JsonWebKey = Reflect.construct(JsonWebKey, [parameters]);
    const thumbprint = key.thumbprint; // For some dumb reason, not calling it here results in undefined

    it('should calculate and return the sha-256 thumbprint of a json web key.', () => {
      expect(thumbprint.toString('base64url')).toEqual('NzbLsXh8uDCcd-6MNwXF4W_7noWXFZAfHkxZsRGC9Xs');
    });
  });

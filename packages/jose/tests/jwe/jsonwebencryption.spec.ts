import { InvalidJoseHeader, InvalidJsonWebEncryption } from '../../lib/exceptions';
...
describe('JSON Web Encryption constructor', () => {
  it('should reject an invalid header.', () => {
    // @ts-expect-error
    expect(() => new JsonWebEncryption({}, plaintext)).toThrow(InvalidJoseHeader);
  });
  ...
  it('should create an instance of a JSON Web Encryption.', () => {
    expect(() => new JsonWebEncryption(protectedHeader, plaintext)).not.toThrow();
  });
});
...
describe('JSON Web Encryption serializeCompact()', () => {
  ...
  it('should reject an invalid key.', async () => {
    await expect(jwe.serializeCompact()).rejects.toThrow('The algorithm "A128KW" requires the use of a JSON Web Key.');
  });
...
describe('JSON Web Encryption deserializeCompact()', () => {
  ...
  it('should reject a malformed token.', async () => {
    await expect(JsonWebEncryption.deserializeCompact('', wrapKey)).rejects.toThrow(InvalidJsonWebEncryption);

    await expect(JsonWebEncryption.deserializeCompact('aaa.bbb', wrapKey)).rejects.toThrow(InvalidJsonWebEncryption);

    await expect(JsonWebEncryption.deserializeCompact('aaa.bbb.ccc.ddd.eee', wrapKey)).rejects.toThrow(
  ...
  it('should validate and decode the JSON Web Signature Token.', async () => {
    await expect(JsonWebEncryption.deserializeCompact(token, wrapKey)).resolves.toMatchObject({

...
describe('Elliptic Curve Public Key', () => {
  ...
  it('should reject an unsupported curve.', () => {
    ...
    // @ts-expect-error
    expect(() => new EcKey({ crv: 'unknown-curve', ...params })).toThrow('Unsupported curve "unknown-curve".');
  });
  ...
  it('should reject an invalid X coordinate.', () => {
    ...
    // @ts-expect-error
    expect(() => new EcKey({ x: 123, ...params })).toThrow('Invalid parameter "x".');
  });
  ...
  it('should reject an invalid Y coordinate.', () => {
    ...
    // @ts-expect-error
    expect(() => new EcKey({ y: 123, ...params })).toThrow('Invalid parameter "y".');
  });
...
describe('Elliptic Curve Private Key', () => {
  ...
  it('should reject keys with a wrong parameter "d".', () => {
    ...
    // @ts-expect-error
    expect(() => new EcKey({ d: 123, ...params })).toThrow('Invalid parameter "d".');
  });
...
describe('EcKey generate()', () => {
  it('should reject an invalid curve.', async () => {
    // @ts-expect-error
    await expect(EcKey.generate('unknown-curve')).rejects.toThrow('Unsupported curve "unknown-curve".');
  });
...
describe('EcKey export()', () => {
  ...
  it('should reject an invalid key.', () => {
    // @ts-expect-error
    expect(() => key.export('invalid', 'pem', 'sec1')).toThrow('Invalid parameter "key".');
  });
  ...
  it('should export a DER X.509 SubjectPublicKeyInfo Elliptic Curve Public Key.', () => {
    expect(key.export('public', 'der')).toEqual(publicKey.export({ format: 'der', type: 'spki' }));
  });

  it('should export a PEM X.509 SubjectPublicKeyInfo Elliptic Curve Public Key.', () => {
    expect(key.export('public', 'pem')).toEqual(publicKey.export({ format: 'pem', type: 'spki' }));
  });

  it('should export a DER SEC.1 Elliptic Curve Private Key.', () => {
    expect(key.export('private', 'der', 'sec1')).toEqual(privateKey.export({ format: 'der', type: 'sec1' }));
  });

  it('should export a PEM SEC.1 Elliptic Curve Private Key.', () => {
    expect(key.export('private', 'pem', 'sec1')).toEqual(privateKey.export({ format: 'pem', type: 'sec1' }));
  });

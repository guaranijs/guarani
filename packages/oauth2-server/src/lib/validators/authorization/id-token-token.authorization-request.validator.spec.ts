...
describe('ID Token & Token Authorization Request Validator', () => {
  ...
  describe('name', () => {
    it('should have "id_token token" as its value.', () => {
      expect(validator.name).toEqual<ResponseType>('id_token token');
    });
  });
  ...
  describe('validate()', () => {
    ...
    it.each(forbiddenResponseModes)('should throw when requesting a forbidden response mode.', async (responseMode) => {
      ...
      await expect(validator.validate(request)).rejects.toThrowWithMessage(
        ...
        `Invalid response_mode "${responseMode}" for response_type "id_token token".`,
      );
    });

    it('should throw when providing an invalid "nonce" parameter.', async () => {

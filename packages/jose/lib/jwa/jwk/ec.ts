...
type SupportedCurves = 'P-256' | 'P-384' | 'P-521'
...
export class ECPublicKey extends JWKAlgorithm implements ECPublicParams {
  public kty: 'EC'
  public crv: string
  public x: string
  public y: string

  public constructor (data: ECPublicParams) {
    super(data)
...
export class ECPrivateKey extends ECPublicKey implements ECPrivateParams {
  public d: string

  public constructor (data: ECPrivateParams) {
    super(data)
...
export function createEcKey (curve: SupportedCurves): [ECPublicKey, ECPrivateKey] {
  ...
  const data = { kty: 'EC', crv: curve, x, y }

  return [new ECPublicKey(data), new ECPrivateKey({ ...data, d })]
}

/* eslint-disable no-redeclare */
export function parseEcKey (data: string, keyType: 'public'): ECPublicKey
export function parseEcKey (data: string, keyType: 'private'): ECPrivateKey
export function parseEcKey (data: string, keyType: 'public' | 'private') {
  if (keyType === 'public') {
    ...
    return new ECPublicKey({ kty: 'EC', crv: curve.id, x, y })
  }
  ...
  if (keyType === 'private') {
    ...
    return new ECPrivateKey({ kty: 'EC', crv: curve.id, x, y, d })
  }

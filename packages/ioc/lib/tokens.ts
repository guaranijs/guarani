 ...
 */
export function isConstructorToken<T>(token: InjectableToken<T>): token is Constructor<T> {
 ...
 */
export function isValueToken<T>(token: InjectableToken<T>): token is string | symbol {
...
export class LazyToken<T> {
  ...
  public resolve(callback: (lazyToken: InjectableToken<T>) => T): T {
    ...
    return new Proxy<any>({}, this.createHandler(callbackResolver)) as T;
  }
  ...
  private createHandler(callbackResolver: () => T): ProxyHandler<object> {
    ...
    for (const method of LazyToken.reflectMethods) {
      const handlerMethod = (...args: any[]): any => {
        return (Reflect[method] as any)(callbackResolver(), ...args.slice(1));
      };

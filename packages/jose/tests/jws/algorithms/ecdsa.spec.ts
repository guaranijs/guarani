...
describe('JWS ECDSA Algorithm ES256', () => {
  ...
  it('should reject a different curve.', async () => {
    await expect(async () => await ES256.sign(message, await EcKey.generate('P-384'))).rejects.toThrow(InvalidKey);
  });
...
describe('JWS ECDSA Algorithm ES384', () => {
  ...
  it('should reject a different curve.', async () => {
    await expect(async () => await ES384.sign(message, await EcKey.generate('P-521'))).rejects.toThrow(InvalidKey);
  });
...
describe('JWS ECDSA Algorithm ES512', () => {
  ...
  it('should reject a different curve.', async () => {
    await expect(async () => await ES512.sign(message, await EcKey.generate('P-256'))).rejects.toThrow(InvalidKey);
  });

...
describe('JSON Web Keyset constructor', () => {
  ...
  it('should reject invalid keysets.', () => {
    ...
    expect(() => new JsonWebKeyset([new EcKey(ecKey, { kid: 'keyid' }), new RsaKey(rsaKey)])).toThrow(
      'One or more keys do not have an ID.'
    );

    expect(() => new JsonWebKeyset([new EcKey(ecKey, { kid: 'keyid' }), new RsaKey(rsaKey, { kid: 'keyid' })])).toThrow(
      'The usage of the same ID for multiple keys in a JWKS is forbidden.'
    );
  });

  it('should create a JsonWebKeyset object.', () => {
    expect(new JsonWebKeyset([new EcKey(ecKey, { kid: 'ec-key' }), new RsaKey(rsaKey, { kid: 'rsa-key' })])).toEqual(
...
describe('JSON Web Keyset getKey()', () => {
  const ecKey = loadAsymmetricKey<any>('EC', 'json', 'public');
  const rsaKey = loadAsymmetricKey<any>('RSA', 'json', 'public');
  const keyset = new JsonWebKeyset([new EcKey(ecKey, { kid: 'ec-key' }), new RsaKey(rsaKey, { kid: 'rsa-key' })]);

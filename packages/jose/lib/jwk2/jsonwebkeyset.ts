...
import { EcKey } from './algorithms/ec/ec.key';
import { OctKey } from './algorithms/oct/oct.key';
import { RsaKey } from './algorithms/rsa/rsa.key';
...
export class JsonWebKeyset {
  ...
  public constructor(keys: JsonWebKey[]) {
    ...
    const ids = keys.map((key) => {
      if (key.kid == null) {
        throw new InvalidJsonWebKeySetException('One or more keys do not have an ID.');
      }
    ...
    if (new Set(ids).size !== ids.length) {
      throw new InvalidJsonWebKeySetException('The usage of the same ID for multiple keys in a JWKS is forbidden.');
    }
  ...
  public static parse(data: any): JsonWebKeyset {
    ...
    const algs: Dict<Constructor<JsonWebKey>> = { EC: EcKey, oct: OctKey, RSA: RsaKey };

...
import { InvalidJoseHeader, InvalidJsonWebSignature } from '../../lib/exceptions';
...
describe('JSON Web Signature serializeCompact()', () => {
  ...
  it('should reject an invalid header.', async () => {
    await expect(new JsonWebSignature([]).serializeCompact(key)).rejects.toThrow(
  ...
  it('should reject an invalid key.', async () => {
    ...
    await expect(jws.serializeCompact()).rejects.toThrow('The algorithm "HS256" requires the use of a JSON Web Key.');
  });
...
describe('JSON Web Signature deserializeCompact()', () => {
  ...
  it('should reject a malformed token.', async () => {
    await expect(JsonWebSignature.deserializeCompact('', key)).rejects.toThrow(InvalidJsonWebSignature);

    await expect(JsonWebSignature.deserializeCompact('aaa.bbb', key)).rejects.toThrow(InvalidJsonWebSignature);

    await expect(JsonWebSignature.deserializeCompact('aaa.bbb.ccc', key)).rejects.toThrow(InvalidJsonWebSignature);
  });
  ...
  it('should reject conflicting algorithms.', async () => {
    await expect(JsonWebSignature.deserializeCompact(token, key, 'none')).rejects.toThrow(
      'The algorithm used to sign this token is invalid. ' + 'Expected "none", got "HS256".'
    );
  });
  ...
  it('should not verify the signature when asked so.', async () => {
    expect(
      JsonWebSignature.decodeCompact(
        'eyJhbGciOiJIUzI1NiJ9.' + 'eyJpYXQiOiAxNzIzMDEwNDU1LCAic3ViIjogIjA3OEJXRERYYXNkY2c4In0.'
      )
    ).toEqual([header, payload]);
  });

  it('should validate and decode the JSON Web Signature Token.', async () => {
    await expect(JsonWebSignature.deserializeCompact(token, key, 'HS256')).resolves.toMatchObject({
      header,
      payload,
    });
  });
...
describe('JSON Web Signature serializeFlattened()', () => {
  ...
  it('should reject an invalid header.', async () => {
    await expect(new JsonWebSignature([]).serializeFlattened(key)).rejects.toThrow(
  ...
  it('should reject an invalid key.', async () => {
    // Only works because the alg is not "none".
    await expect(jws.serializeFlattened()).rejects.toThrow('The algorithm "HS256" requires the use of a JSON Web Key.');
  });

  it('should encode a JOSE Header and a Payload into a JWS Flattened Token.', async () => {
    await expect(jws.serializeFlattened(key)).resolves.toMatchObject<JWSFlattenedSerialization>({
